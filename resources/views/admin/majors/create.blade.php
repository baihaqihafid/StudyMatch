...
<form method="POST" action="{{ route('admin.majors.store') }}"
    ...
    <div>
        ...
        <select name="category" class="w-full border border-gray-300 rounded-lg px-4 py-2">
            <option value="Saintek" {{ old('category') === 'Saintek' ? 'selected' : '' }}>Saintek</option>
            <option value="Soshum" {{ old('category') === 'Soshum' ? 'selected' : '' }}>Soshum</option>
        </select>
    </div>
    ...
    <div>
        ...
        <textarea name="career_prospects" rows="2"
                  class="w-full border border-gray-300 rounded-lg px-4 py-2">{{ old('career_prospects') }}</textarea>
    </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" name="values[{{ $criteria->id }}]"
                       min="1" max="5" value="{{ old('values.'.$criteria->id, 3) }}"
